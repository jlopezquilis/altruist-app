...
import com.altruist.viewmodel.LoginViewModel_API

//Anotación de Jetpack Compose que indica que genera parte de la UI
@Composable
fun LoginScreen_API(viewModel: LoginViewModel_API = viewModel()) { //Porque = viewModel()?
    ...
    Column(modifier = Modifier
        ...
        loginResult?.let { result ->
            Spacer(modifier = Modifier.height(16.dp))
            result.onSuccess {
                Text("Bienvenido/a, ${it.name}!", color = MaterialTheme.colorScheme.error)
            }.onFailure {
